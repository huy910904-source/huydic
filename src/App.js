...
export default function App() {
  ...
  const handleNewAudioUpload = (e) => {
    if (e.target.files[0]) setNewAudioUrl(URL.createObjectURL(e.target.files[0]));
  };
  const handleNewJsonUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        try { setNewJsonData(JSON.parse(event.target.result)); } catch (error) { alert("Lỗi đọc file JSON!"); }
      };
  ...
  const createNewLesson = () => {
    ...
    setNewJsonData(null); setNewAudioUrl(null); setNewFileName("");
    setActiveLessonId(newLesson.id); setActiveTab("DICTATION"); resetDictationState();
  };

  const deleteLesson = (id) => {
    if (window.confirm("Bạn có chắc muốn xóa bài học này khỏi thư viện? Các câu lưu của bài này cũng sẽ bị xóa.")) {
  ...
  const playSavedSentence = (lessonId, sentenceIdx) => {
    if (!sessionAudioUrls[lessonId]) {
      alert("Bài học này chưa được kết nối Audio trong phiên làm việc. Vui lòng quay lại Thư viện và bấm 'Học tiếp' để kết nối MP3 trước.");
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "1000px", margin: "0 auto", fontFamily: "sans-serif", backgroundColor: "#0f172a", color: "#e2e8f0", minHeight: "100vh" }}>
      ...
      {/* TABS HEADER CHÍNH */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", borderBottom: "2px solid #334155", paddingBottom: "10px", marginBottom: "20px" }}>
        <button onClick={() => setActiveTab("LIBRARY")} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "LIBRARY" ? "#10b981" : "transparent", color: activeTab === "LIBRARY" ? "#0f172a" : "#94a3b8", border: "none", borderRadius: "8px", cursor: "pointer", transition: "all 0.3s" }}>
          📚 Thư Viện ({library.length})
        </button>
        <button onClick={() => { if (activeLessonId) setActiveTab("DICTATION"); else alert("Vui lòng chọn 1 bài học từ Thư Viện trước!"); }} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "DICTATION" ? "#38bdf8" : "transparent", color: activeTab === "DICTATION" ? "#0f172a" : "#94a3b8", border: "none", borderRadius: "8px", cursor: "pointer", transition: "all 0.3s", opacity: activeLessonId ? 1 : 0.5 }}>
          🎧 Luyện Nghe
        </button>
        <button onClick={() => setActiveTab("SAVED")} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "SAVED" ? "#f59e0b" : "transparent", color: activeTab === "SAVED" ? "#fff" : "#94a3b8", border: "none", borderRadius: "8px", cursor: "pointer", transition: "all 0.3s" }}>
          ⭐ Câu Đã Lưu ({savedSentences.length})
        </button>
        <button onClick={() => setActiveTab("VOCAB")} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "VOCAB" ? "#a855f7" : "transparent", color: activeTab === "VOCAB" ? "#fff" : "#94a3b8", border: "none", borderRadius: "8px", cursor: "pointer", transition: "all 0.3s" }}>
          ...
          <div style={{ background: "#1e293b", padding: "20px", borderRadius: "12px", marginBottom: "30px", border: "2px dashed #334155" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "15px", alignItems: 'flex-start' }}>
              <div style={{ flex: 1, minWidth: "200px" }}>
                <p style={{ margin: "0 0 5px 0", fontSize: "14px", color: "#94a3b8" }}>1. Audio (.mp3)</p>
                <input type="file" accept=".mp3, .wav, audio/mp3, audio/wav, audio/*" onChange={handleNewAudioUpload} style={{ color: "#fff", padding: '8px', background: '#0f172a', borderRadius: '6px', width: '100%' }} />
              </div>
              <div style={{ flex: 1, minWidth: "200px" }}>
                <p style={{ margin: "0 0 5px 0", fontSize: "14px", color: "#94a3b8" }}>2. Script (.json)</p>
                <input type="file" accept=".json" onChange={handleNewJsonUpload} style={{ color: "#fff", padding: '8px', background: '#0f172a', borderRadius: '6px', width: '100%' }} />
              </div>
              
              {/* TÍNH NĂNG MỚI: CHO PHÉP ĐỔI TÊN BÀI HỌC */}
              <div style={{ flex: 1, minWidth: "200px" }}>
                <p style={{ margin: "0 0 5px 0", fontSize: "14px", color: "#94a3b8" }}>3. Tên bài học (Sửa tùy ý)</p>
                <input 
                  type="text" 
                  value={newFileName} 
                  onChange={(e) => setNewFileName(e.target.value)} 
                  placeholder="Nhập tên dễ nhớ..."
                  style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #334155', background: '#0f172a', color: '#fff', outline: 'none' }}
                />
              </div>
            </div>

            <button onClick={createNewLesson} disabled={!newJsonData || !newAudioUrl} style={{ marginTop: "20px", padding: "12px 20px", background: !newJsonData || !newAudioUrl ? "#334155" : "#10b981", color: "#fff", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: !newJsonData || !newAudioUrl ? "not-allowed" : "pointer", width: '100%' }}>
              Tạo và Học Ngay 🚀
            </button>
          </div>
                ...
                <div key={lesson.id} style={{ background: "#1e293b", padding: "20px", borderRadius: "12px", borderLeft: "4px solid #38bdf8", position: "relative" }}>
                  ...
                  {sessionAudioUrls[lesson.id] ? (
                    ...
                    <div style={{ background: "#334155", padding: "10px", borderRadius: "8px" }}>
                      <p style={{ margin: "0 0 8px 0", fontSize: "13px", color: "#fca5a5" }}>⚠ Cần liên kết lại file MP3 để học tiếp:</p>
                      <input type="file" accept=".mp3, .wav, audio/*" onChange={(e) => {
                          if (e.target.files[0]) setSessionAudioUrls((prev) => ({ ...prev, [lesson.id]: URL.createObjectURL(e.target.files[0]) }));
      ...
      {activeTab === "DICTATION" && activeLesson && (
        <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
            <span style={{ fontSize: "18px", color: "#e2e8f0", fontWeight: "bold" }}>{activeLesson.name}</span>

            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <span style={{ fontSize: "15px", color: "#94a3b8" }}>Câu</span>
              <select value={activeLesson.currentIdx} onChange={(e) => jumpToSentence(parseInt(e.target.value, 10))} style={{ padding: "6px 10px", borderRadius: "8px", background: "#1e293b", color: "#38bdf8", border: "1px solid #334155", fontSize: "16px", fontWeight: "bold", outline: "none", cursor: "pointer" }}>
                {activeLesson.data.map((_, idx) => ( <option key={idx} value={idx}>{idx + 1}</option> ))}
              </select>
              <span style={{ fontSize: "15px", color: "#94a3b8" }}>/ {activeLesson.data.length}</span>
              ...
              <button 
                ...
                style={{ 
                  ...
                  fontSize: '24px', padding: '0 5px', color: isCurrentSaved ? '#f59e0b' : '#64748b', 
                  transition: 'transform 0.2s', transform: isCurrentSaved ? 'scale(1.1)' : 'scale(1)' 
                }}
              >
                {isCurrentSaved ? "⭐" : "☆"}
              </button>
          ...
          <div style={{ background: "#1e293b", padding: "15px", borderRadius: "12px", marginBottom: "20px" }}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", marginBottom: "15px" }}>
              <button onClick={playSegment} style={{ padding: "10px 15px", borderRadius: "8px", background: "#38bdf8", color: "#0f172a", border: "none", fontWeight: "bold", cursor: "pointer" }}>▶ Phát câu này</button>
              <button onClick={() => playRange(Math.max(0, activeLesson.currentIdx - 1), activeLesson.currentIdx)} style={{ padding: "10px 15px", borderRadius: "8px", background: "#475569", color: "#fff", border: "1px solid #64748b", fontWeight: "bold", cursor: "pointer" }}>🔗 Nối câu (n-1)</button>
              <button onClick={() => playRange(0, activeLesson.currentIdx)} style={{ padding: "10px 15px", borderRadius: "8px", background: "#475569", color: "#fff", border: "1px solid #64748b", fontWeight: "bold", cursor: "pointer" }}>⏮ Nghe từ câu 1</button>
              <button onClick={rewindAudio} style={{ padding: "10px 15px", borderRadius: "8px", background: "#334155", color: "#e2e8f0", border: "none", cursor: "pointer" }}>⏪ Lùi 2s</button>
            </div>

            <div style={{ display: "flex", gap: "15px", alignItems: "center", borderTop: "1px solid #334155", paddingTop: "10px" }}>
              <label style={{ display: "flex", alignItems: "center", gap: "5px", cursor: "pointer", fontSize: "14px", color: "#94a3b8" }}>
          ...
          <textarea
            ...
            style={{ width: "100%", height: "100px", fontSize: "18px", padding: "15px", borderRadius: "12px", border: isSuccess ? "2px solid #22c55e" : "2px solid #334155", background: "#1e293b", color: "#fff", outline: "none", resize: "vertical", transition: "border 0.3s" }}
          />
          ...
          <div ref={resultBoxRef} style={{ marginTop: "20px", border: "2px solid #334155", padding: "20px", borderRadius: "12px", background: "#1e293b", minHeight: "120px", maxHeight: "300px", overflowY: "auto" }}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", fontSize: "20px", lineHeight: "1.8" }}>
              {!showFeedback && <span style={{ color: "#64748b", fontStyle: "italic" }}>Bản dịch sẽ tự động hiện khi bạn hoàn thành câu...</span>}
              ...
              {showFeedback && currentSegment && currentSegment.transcript.split(/\s+/).filter(Boolean).map((word, i) => {
                  ...
                  for (let k = 0; k < rawTranscriptWords.length; k++) {
                    if (normalize(userWords[k]) !== normalize(rawTranscriptWords[k])) {
                      currentErrorIdx = k;
                      break;
                    }
                  }
                  ...
                  if (i < currentErrorIdx || currentErrorIdx === -1) {
                    return <span key={i} style={{ color: "#22c55e", fontWeight: "bold" }}>{word}</span>;
                  }
                  if (i === currentErrorIdx && !isSuccess) {
                    ...
                    return (
                      <span key={i} style={{ background: "#450a0a", border: "1px solid #f87171", padding: "2px 10px", borderRadius: "6px", display: "inline-flex", alignItems: "center", gap: "8px" }}>
                        {wrongUserWord && <span style={{ color: "#ef4444", textDecoration: "line-through", fontSize: "16px" }}>{wrongUserWord}</span>}
                        <span style={{ color: "#fbbf24", fontFamily: "monospace", fontWeight: "bold", letterSpacing: "2px" }}>{hintStr}</span>
                      </span>
                    );
                  }
                  if (i > currentErrorIdx) return <span key={i} style={{ color: "#475569" }}>___</span>;
                  return null;
                })}
            </div>
      ...
      {activeTab === "SAVED" && (
        <div style={{ background: "#1e293b", padding: "20px", borderRadius: "12px" }}>
          <h3 style={{ color: "#f59e0b", marginTop: 0, borderBottom: '1px solid #334155', paddingBottom: '10px' }}>
            ⭐ Danh Sách Ôn Tập ({savedSentences.length} câu)
          </h3>
          
          {savedSentences.length === 0 ? (
            <p style={{ color: "#64748b", fontStyle: "italic", textAlign: 'center', padding: '20px' }}>
              Bạn chưa lưu câu nào. Khi đang luyện nghe, hãy bấm vào biểu tượng ngôi sao (☆) góc phải để lưu câu khó vào đây nhé!
            </p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
              {savedSentences.map((item) => (
                <div key={item.id} style={{ background: "#0f172a", padding: "15px", borderRadius: "10px", borderLeft: "4px solid #f59e0b", position: 'relative' }}>
                  <button onClick={() => removeSavedSentence(item.id)} style={{ position: "absolute", top: "15px", right: "15px", background: "transparent", border: "none", color: "#ef4444", fontSize: "18px", cursor: "pointer" }} title="Xóa khỏi danh sách">✖</button>
                  
                  <div style={{ marginBottom: '10px' }}>
                    <span style={{ background: 'rgba(245, 158, 11, 0.2)', color: '#fcd34d', padding: '4px 8px', borderRadius: '4px', fontSize: '12px', fontWeight: 'bold' }}>{item.lessonName}</span>
                    <span style={{ color: '#94a3b8', fontSize: '13px', marginLeft: '10px' }}>Câu {item.sentenceIdx + 1}</span>
                  </div>
                  
                  <p style={{ margin: "0 0 8px 0", fontSize: "18px", color: "#e2e8f0", paddingRight: '30px', lineHeight: '1.5' }}>
                    {item.transcript}
                  </p>
                  <p style={{ margin: "0 0 15px 0", color: "#94a3b8", fontStyle: 'italic', fontSize: "14px" }}>
                    🇻🇳 {item.translation}
                  </p>
                  
                  <button 
                    onClick={() => playSavedSentence(item.lessonId, item.sentenceIdx)} 
                    style={{ padding: "8px 16px", background: "linear-gradient(135deg, #3b82f6, #2563eb)", color: "#fff", border: "none", borderRadius: "6px", fontWeight: "bold", cursor: "pointer" }}
                  >
      ...
      {activeTab === "VOCAB" && (
        <div style={{ background: "#1e293b", padding: "20px", borderRadius: "12px" }}>
          ...
          <h3 style={{ color: "#38bdf8", borderBottom: '1px solid #334155', paddingBottom: '10px' }}>📚 Danh sách từ vựng của bạn</h3>
          {vocabList.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
              {vocabList.map((v) => (
                <div key={v.id} style={{ background: "#0f172a", padding: "15px", borderRadius: "8px", borderLeft: "4px solid #a855f7", position: "relative" }}>
                  ...
                  <p style={{ margin: "0 0 5px 0", fontSize: "18px", fontWeight: "bold", color: "#e2e8f0", paddingRight: '20px' }}>{v.word}</p>
