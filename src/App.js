...
export default function App() {
  ...
  // 1. TẢI DỮ LIỆU TỪ LOCAL & INDEXED DB
  useEffect(() => {
  ...
  useEffect(() => localStorage.setItem("my_error_log", JSON.stringify(errorLog)), [errorLog]);
  useEffect(() => localStorage.setItem("my_saved_sentences", JSON.stringify(savedSentences)), [savedSentences]);

  useEffect(() => {
    if (showFeedback && resultBoxRef.current) resultBoxRef.current.scrollTop = resultBoxRef.current.scrollHeight;
  }, [input, showFeedback]);
  ...
  const hardSentencesInCurrentLesson = [...new Set(errorLog.filter(e => e.lessonId === activeLessonId).map(e => e.sentenceIdx))].sort((a, b) => a - b);
  const currentSentenceId = activeLesson ? `${activeLesson.id}_${activeLesson.currentIdx}` : "";
  ...
  const triggerShake = () => {
    setIsShaking(true); setTimeout(() => setIsShaking(false), 400);
  };
  ...
  const logError = (targetWord, userWord) => {
    ...
    if (!userWord) {
      errorType = "Thiếu từ"; userWord = "(Thiếu từ)";
    } else {
      const cleanTarget = normalize(targetWord);
      const cleanUser = normalize(userWord);
      if (cleanTarget.startsWith(cleanUser.substring(0, 3)) || cleanUser.startsWith(cleanTarget.substring(0, 3))) {
    ...
    setErrorLog(prev => {
      const newLog = [...prev];
      const existing = newLog.find(e => e.lessonId === activeLessonId && e.sentenceIdx === activeLesson.currentIdx && e.targetWord === targetWord);
      if (existing) {
        existing.count += 1; existing.lastUserWord = userWord; existing.type = errorType;
      } else {
        newLog.push({
          id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
          lessonId: activeLessonId, lessonName: activeLesson.name, sentenceIdx: activeLesson.currentIdx,
          targetWord: targetWord, lastUserWord: userWord, type: errorType, count: 1, timestamp: Date.now()
        });
      }
      return newLog;
    });
  ...
  // CHUYỂN CÂU
  // ==========================================
  const resetDictationState = () => { setInput(""); setAttemptsPerWord({}); setShowFeedback(false); setIsSuccess(false); };

  const jumpToSentence = (newIdx) => {
    if (!activeLesson) return;
    setLibrary((prev) => prev.map((lesson) => lesson.id === activeLesson.id ? { ...lesson, currentIdx: newIdx } : lesson));
    resetDictationState(); setKeyForAnimation(prev => prev + 1); autoPlayRef.current = true;
  };

  const reviewErrorSentence = (lessonId, sentenceIdx) => {
    setActiveLessonId(lessonId); setReviewMode(false);
    setLibrary((prev) => prev.map((lesson) => lesson.id === lessonId ? { ...lesson, currentIdx: sentenceIdx } : lesson));
    resetDictationState(); setActiveTab("DICTATION"); setKeyForAnimation(prev => prev + 1);
  };
  ...
  const nextSentence = () => {
    ...
    if (reviewHardMode && hardSentencesInCurrentLesson.length > 0) {
      const currentHardIndex = hardSentencesInCurrentLesson.indexOf(activeLesson.currentIdx);
      if (currentHardIndex !== -1 && currentHardIndex < hardSentencesInCurrentLesson.length - 1) {
        nextIdx = hardSentencesInCurrentLesson[currentHardIndex + 1];
      } else if (currentHardIndex === -1) {
        nextIdx = hardSentencesInCurrentLesson.find(idx => idx > activeLesson.currentIdx) ?? -1;
      }
    } else {
      if (activeLesson.currentIdx < activeLesson.data.length - 1) nextIdx = activeLesson.currentIdx + 1;
    }

    if (nextIdx !== -1) jumpToSentence(nextIdx);
    else alert(reviewHardMode ? "🎉 Tuyệt vời! Bạn đã ôn xong toàn bộ các câu khó!" : "🎉 Bạn đã hoàn thành toàn bộ bài nghe này!");
  ...
  // ==========================================
  const handleNewAudioUpload = (e) => { if (e.target.files[0]) setNewAudioFile(e.target.files[0]); };
  const handleNewJsonUpload = (e) => {
    const file = e.target.files[0];
    if (file) {
      setNewFileName(file.name.replace(".json", ""));
      const reader = new FileReader();
      reader.onload = (event) => { try { setNewJsonData(JSON.parse(event.target.result)); } catch (error) { alert("Lỗi đọc JSON!"); } };
  ...
  const createNewLesson = async () => {
    ...
    setActiveLessonId(newId); setActiveTab("DICTATION"); resetDictationState(); setKeyForAnimation(prev => prev + 1);
  };

  const deleteLesson = async (id) => {
    if (window.confirm("Xóa vĩnh viễn bài này và mọi dữ liệu thống kê liên quan?")) {
  ...
  const toggleSaveSentence = () => {
    ...
    if (isCurrentSaved) {
      setSavedSentences((prev) => prev.filter((s) => s.id !== currentSentenceId));
    } else {
      setSavedSentences((prev) => [...prev, {
          id: currentSentenceId, lessonId: activeLesson.id, lessonName: activeLesson.name,
          sentenceIdx: activeLesson.currentIdx, transcript: currentSegment.transcript, translation: currentSegment.translation,
      }]);
    }
  };
  ...
  const playSavedSentence = (lessonId, sentenceIdx) => {
    if (!sessionAudioUrls[lessonId]) {
      alert("Cần nối lại Audio ở tab Thư Viện trước!"); setActiveTab("LIBRARY"); return;
    }
    setActiveLessonId(lessonId); updateProgress(lessonId, sentenceIdx);
    resetDictationState(); setActiveTab("DICTATION"); autoPlayRef.current = true;
  };
  ...
  const handleCheck = () => {
    ...
    setIsSuccess(false); setShowFeedback(true); triggerShake();
  };
  ...
  const handleSurrenderWord = () => {
    ...
    if (errorIdx !== -1) {
      ...
      const newInputArray = userWords.slice(0, errorIdx); newInputArray.push(correctWord);
      setInput(newInputArray.join(" ") + " ");
      logError(rawTranscriptWords[errorIdx], userWords[errorIdx]);
      triggerShake(); setTimeout(() => handleCheck(), 100);
    }
  };
  ...
  return (
    <div className="animated-bg" style={{ minHeight: "100vh", color: "#e2e8f0", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ padding: "20px", maxWidth: "1100px", margin: "0 auto" }}>
        ...
        {/* TABS HEADER CHÍNH */}
        <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", borderBottom: "2px solid rgba(255,255,255,0.1)", paddingBottom: "15px", marginBottom: "25px" }}>
          <button className="btn-hover" onClick={() => setActiveTab("LIBRARY")} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "LIBRARY" ? "#10b981" : "rgba(255,255,255,0.05)", color: activeTab === "LIBRARY" ? "#0f172a" : "#94a3b8", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", cursor: "pointer" }}>📚 Thư Viện</button>
          <button className="btn-hover" onClick={() => { if (activeLessonId) { setActiveTab("DICTATION"); setKeyForAnimation(prev=>prev+1); } else alert("Chọn bài học trước!"); }} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "DICTATION" ? "#38bdf8" : "rgba(255,255,255,0.05)", color: activeTab === "DICTATION" ? "#0f172a" : "#94a3b8", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", cursor: "pointer", opacity: activeLessonId ? 1 : 0.5 }}>🎧 Luyện Nghe</button>
          <button className="btn-hover" onClick={() => setActiveTab("STATS")} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "STATS" ? "#ef4444" : "rgba(255,255,255,0.05)", color: activeTab === "STATS" ? "#fff" : "#94a3b8", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", cursor: "pointer" }}>📊 Lỗi ({errorLog.length})</button>
          <button className="btn-hover" onClick={() => setActiveTab("SAVED")} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "SAVED" ? "#f59e0b" : "rgba(255,255,255,0.05)", color: activeTab === "SAVED" ? "#fff" : "#94a3b8", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", cursor: "pointer" }}>⭐ Đã Lưu ({savedSentences.length})</button>
          <button className="btn-hover" onClick={() => setActiveTab("VOCAB")} style={{ flex: 1, minWidth: '120px', padding: "12px", fontSize: "16px", fontWeight: "bold", background: activeTab === "VOCAB" ? "#a855f7" : "rgba(255,255,255,0.05)", color: activeTab === "VOCAB" ? "#fff" : "#94a3b8", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", cursor: "pointer" }}>📖 Sổ Từ</button>
        </div>
        ...
        {activeTab === "LIBRARY" && (
          <div className="fade-in">
            <div className="glass-panel" style={{ padding: "25px", borderRadius: "16px", marginBottom: "30px", border: "2px dashed rgba(56, 189, 248, 0.4)" }}>
              <h3 style={{ color: "#38bdf8", marginTop: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>➕ Thêm Bài Học Mới</h3>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "20px", alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: "200px" }}>
                  <p style={{ margin: "0 0 8px 0", fontSize: "14px", color: "#cbd5e1" }}>3. Tên bài học (Tùy chỉnh)</p>
                  <input type="text" value={newFileName} onChange={(e) => setNewFileName(e.target.value)} placeholder="Nhập tên dễ nhớ..." style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.2)', color: '#fff', outline: 'none' }} />
                </div>
              </div>
            ...
            {library.length === 0 ? <p style={{ color: "#64748b", fontStyle: "italic", textAlign: 'center' }}>Thư viện trống.</p> : (
              <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                {library.map((lesson) => (
                  <div key={lesson.id} className="glass-panel" style={{ padding: "20px", borderRadius: "16px", borderLeft: "5px solid #38bdf8", position: "relative" }}>
                    ...
                    <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: "10px", height: "10px", overflow: "hidden", marginBottom: "12px", boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.5)' }}>
                      <div style={{ width: `${(lesson.currentIdx / lesson.data.length) * 100}%`, height: "100%", background: "linear-gradient(90deg, #38bdf8, #818cf8)", transition: "width 0.5s ease" }}></div>
                    </div>
                    ...
                    {sessionAudioUrls[lesson.id] ? (
                      <button className="btn-hover" onClick={() => { setActiveLessonId(lesson.id); setActiveTab("DICTATION"); resetDictationState(); setKeyForAnimation(prev=>prev+1); }} style={{ padding: "10px 24px", background: "linear-gradient(135deg, #3b82f6, #2563eb)", color: "#fff", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: "pointer" }}>▶ Tiếp tục học</button>
                    ) : (
                      <div style={{ background: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.3)", padding: "12px", borderRadius: "10px" }}>
                        <p style={{ margin: "0 0 8px 0", fontSize: "13px", color: "#fca5a5" }}>⚠ Cần nối lại MP3:</p>
          ...
          <div key={keyForAnimation} className="fade-in" style={{ display: 'flex', gap: '20px', flexDirection: 'column' }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "5px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "10px", background: 'rgba(0,0,0,0.2)', padding: '5px 15px', borderRadius: '20px' }}>
                ...
                {/* DROPDOWN CHỌN CÂU */}
                <select className="btn-hover" value={activeLesson.currentIdx} onChange={(e) => jumpToSentence(parseInt(e.target.value, 10))} style={{ padding: "6px 12px", borderRadius: "8px", background: reviewHardMode ? "rgba(239, 68, 68, 0.2)" : "rgba(255,255,255,0.1)", color: reviewHardMode ? "#fca5a5" : "#38bdf8", border: reviewHardMode ? "1px solid rgba(239, 68, 68, 0.4)" : "1px solid rgba(255,255,255,0.2)", fontSize: "16px", fontWeight: "bold", outline: "none", cursor: "pointer" }}>
                  {reviewHardMode 
                    ? hardSentencesInCurrentLesson.map(idx => <option key={idx} value={idx} style={{color: '#000'}}>{idx + 1} (Khó)</option>)
                ...
                <button onClick={toggleSaveSentence} title="Lưu câu này" style={{ background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '24px', padding: '0 5px', color: isCurrentSaved ? '#f59e0b' : '#64748b', transition: 'transform 0.2s', transform: isCurrentSaved ? 'scale(1.2)' : 'scale(1)' }}>
                  {isCurrentSaved ? "⭐" : "☆"}
                </button>
              </div>
            </div>
            ...
            <div className="glass-panel" style={{ padding: "20px", borderRadius: "16px", border: reviewHardMode ? "1px solid rgba(239, 68, 68, 0.3)" : "1px solid rgba(255, 255, 255, 0.05)" }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginBottom: "15px" }}>
                <button className="btn-hover" onClick={playSegment} style={{ padding: "12px 18px", borderRadius: "10px", background: "linear-gradient(135deg, #38bdf8, #0ea5e9)", color: "#fff", border: "none", fontWeight: "bold", cursor: "pointer" }}>▶ Phát câu này</button>
                <button className="btn-hover" onClick={() => playRange(Math.max(0, activeLesson.currentIdx - 1), activeLesson.currentIdx)} style={{ padding: "12px 18px", borderRadius: "10px", background: "rgba(255,255,255,0.1)", color: "#fff", border: "1px solid rgba(255,255,255,0.2)", fontWeight: "bold", cursor: "pointer" }}>🔗 Nghe (n-1) & (n)</button>
                <button className="btn-hover" onClick={() => playRange(0, activeLesson.currentIdx)} style={{ padding: "12px 18px", borderRadius: "10px", background: "rgba(255,255,255,0.1)", color: "#fff", border: "1px solid rgba(255,255,255,0.2)", fontWeight: "bold", cursor: "pointer" }}>⏮ Từ đầu đến (n)</button>
                <button className="btn-hover" onClick={rewindAudio} style={{ padding: "12px 18px", borderRadius: "10px", background: "rgba(0,0,0,0.3)", color: "#e2e8f0", border: "none", cursor: "pointer" }}>⏪ Lùi 2s</button>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '12px', paddingBottom: '15px', borderBottom: "1px solid rgba(255,255,255,0.1)", marginBottom: '15px' }}>
                ...
                <button className="btn-hover" onClick={() => playRange(customStartIdx, customEndIdx)} style={{ padding: "8px 18px", borderRadius: "8px", background: "linear-gradient(135deg, #a855f7, #7e22ce)", color: "#fff", border: "none", fontWeight: "bold", cursor: "pointer", boxShadow: '0 4px 10px rgba(126, 34, 206, 0.3)' }}>▶ Phát đoạn</button>
              </div>
              ...
              <div style={{ display: "flex", flexWrap: 'wrap', gap: "25px", alignItems: "center" }}>
                <label className="btn-hover" style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "15px", color: "#34d399", fontWeight: 'bold', padding: '5px 10px', background: 'rgba(52, 211, 153, 0.1)', borderRadius: '8px' }}>
                  <input type="checkbox" checked={isAutoLoop} onChange={(e) => setIsAutoLoop(e.target.checked)} style={{ transform: 'scale(1.2)' }} /> 🔁 Tự động lặp
                </label>
                ...
                <label className="btn-hover" style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "15px", color: "#fbbf24", fontWeight: 'bold', padding: '5px 10px', background: 'rgba(251, 191, 36, 0.1)', borderRadius: '8px' }}>
                  <input type="checkbox" checked={showFullScript} onChange={(e) => setShowFullScript(e.target.checked)} style={{ transform: 'scale(1.2)' }} /> 📜 Karaoke Script
                </label>

                <label className="btn-hover" style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "15px", color: reviewHardMode ? "#ef4444" : "#cbd5e1", fontWeight: 'bold', padding: '5px 10px', background: reviewHardMode ? 'rgba(239, 68, 68, 0.1)' : 'rgba(255,255,255,0.05)', borderRadius: '8px', border: reviewHardMode ? '1px solid rgba(239,68,68,0.5)' : '1px solid transparent', transition: 'all 0.3s' }}>
                  <input type="checkbox" checked={reviewHardMode} onChange={(e) => {
                    setReviewMode(e.target.checked);
                    if (e.target.checked && hardSentencesInCurrentLesson.length > 0) jumpToSentence(hardSentencesInCurrentLesson[0]);
                    else if (e.target.checked) { alert("Chưa có lỗi sai nào trong bài này!"); setReviewMode(false); }
            ...
            <div style={{ display: 'grid', gridTemplateColumns: showFullScript ? '1fr 1fr' : '1fr', gap: '25px', transition: 'all 0.4s ease' }}>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <textarea 
                  ...
                  style={{ width: "100%", height: "130px", fontSize: "18px", padding: "18px", borderRadius: "16px", border: isSuccess ? "2px solid #22c55e" : "2px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.2)", color: "#fff", outline: "none", resize: "vertical", transition: "all 0.3s ease", boxShadow: isSuccess ? "0 0 25px rgba(34, 197, 94, 0.3)" : "inset 0 2px 4px rgba(0,0,0,0.3)" }}
                />

                <div style={{ display: "flex", gap: "15px", marginTop: "20px" }}>
                  <button className={`btn-hover ${isSuccess ? "pulse-btn" : ""}`} onClick={isSuccess ? nextSentence : handleCheck} style={{ flex: 2, padding: "16px", fontSize: "18px", background: isSuccess ? "linear-gradient(135deg, #3b82f6, #2563eb)" : "linear-gradient(135deg, #22c55e, #16a34a)", color: "white", border: "none", borderRadius: "12px", cursor: "pointer", fontWeight: "bold", textShadow: '0 1px 2px rgba(0,0,0,0.3)' }}>
                ...
                <div ref={resultBoxRef} className="glass-panel" style={{ marginTop: "25px", padding: "25px", borderRadius: "16px", minHeight: "130px", maxHeight: showFullScript ? "300px" : "350px", overflowY: "auto" }}>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", fontSize: "22px", lineHeight: "1.8" }}>
                    {!showFeedback && <span style={{ color: "#64748b", fontStyle: "italic", fontSize: '18px' }}>Bản dịch sẽ tự động hiển thị khi bạn hoàn thành...</span>}
                    {showFeedback && currentSegment && currentSegment.transcript.split(/\s+/).filter(Boolean).map((word, i) => {
                      ...
                      if (i < currentErrorIdx || currentErrorIdx === -1) return <span key={i} style={{ color: "#34d399", fontWeight: "bold", textShadow: "0 0 8px rgba(52, 211, 153, 0.4)" }}>{word}</span>;
                      ...
                      if (i === currentErrorIdx && !isSuccess) {
                        ...
                        return (
                          <span className="pop-in" key={i} style={{ background: "rgba(220, 38, 38, 0.2)", border: "1px solid rgba(248, 113, 113, 0.5)", padding: "4px 12px", borderRadius: "8px", display: "inline-flex", alignItems: "center", gap: "10px" }}>
                  ...
                  {isSuccess && (
                    <div className="pop-in" style={{ marginTop: "25px", borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: "20px" }}>
                      <p className="floating" style={{ color: "#34d399", fontWeight: "900", fontSize: "22px", margin: "0 0 15px 0", display: 'inline-block' }}><span style={{fontSize: '26px', marginRight: '8px'}}>🎉</span> Hoàn hảo! Xuất sắc!</p>
                      <div style={{ background: 'rgba(139, 92, 246, 0.1)', borderLeft: '4px solid #8b5cf6', padding: '15px', borderRadius: '0 8px 8px 0' }}>
                        <p style={{ color: "#c4b5fd", fontStyle: "italic", margin: 0, fontSize: '18px', lineHeight: '1.6' }}>🇻🇳 Dịch: {currentSegment?.translation}</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {showFullScript && (
                <div className="fade-in glass-panel" style={{ borderRadius: "16px", padding: "20px", maxHeight: "450px", overflowY: "auto", display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ position: 'sticky', top: '-20px', background: 'rgba(15, 23, 42, 0.95)', backdropFilter: 'blur(10px)', padding: '15px 0 10px 0', marginBottom: '10px', zIndex: 10, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                    <h4 style={{ margin: 0, color: '#fbbf24', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '18px' }}><span style={{fontSize: '22px'}}>📜</span> Script Tracker (Karaoke)</h4>
                  </div>
                  {activeLesson.data.map((seg, idx) => {
                    const isPlaying = idx === currentPlayingIdx;
                    return (
                      <p key={idx} id={`script-line-${idx}`} className="btn-hover" style={{ margin: 0, padding: '12px 15px', borderRadius: '10px', fontSize: '18px', lineHeight: '1.6', color: isPlaying ? '#0f172a' : '#cbd5e1', background: isPlaying ? 'linear-gradient(90deg, #fbbf24, #f59e0b)' : 'rgba(255,255,255,0.03)', fontWeight: isPlaying ? 'bold' : 'normal', transform: isPlaying ? 'scale(1.02)' : 'scale(1)', cursor: 'pointer' }} onClick={() => playRange(idx, idx)}>
                        <span style={{ fontSize: '12px', marginRight: '10px', opacity: isPlaying ? 0.8 : 0.4, background: isPlaying ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.1)', padding: '2px 6px', borderRadius: '4px' }}>{idx + 1}</span>
                        {seg.transcript}
                      </p>
                    );
                  })}
        ...
        {activeTab === "STATS" && (
          <div className="fade-in glass-panel" style={{ padding: "30px", borderRadius: "16px" }}>
            <h3 style={{ color: "#ef4444", marginTop: 0, fontSize: '24px', display: 'flex', alignItems: 'center', gap: '8px' }}><span>📊</span> Bảng Phân Tích Lỗi Sai</h3>
            {errorLog.length === 0 ? (
              <div style={{ textAlign: "center", padding: "50px", background: "rgba(0,0,0,0.2)", borderRadius: "12px" }}>
                <span style={{ fontSize: "40px" }}>🏆</span>
                <p style={{ color: "#34d399", fontSize: "18px", fontWeight: "bold" }}>Tuyệt vời! Bạn chưa có lỗi sai nào được ghi nhận.</p>
              </div>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table className="error-table">
                  ...
                  <tbody>
                    {[...errorLog].sort((a, b) => b.count - a.count).map((err) => (
                      <tr key={err.id}>
                        <td style={{ color: "#cbd5e1" }}>{err.lessonName}</td>
                        <td><span style={{ background: "rgba(255,255,255,0.1)", padding: "2px 8px", borderRadius: "4px", color: "#38bdf8" }}>Câu {err.sentenceIdx + 1}</span></td>
                        <td style={{ color: "#34d399", fontWeight: "bold" }}>{err.targetWord}</td>
                        <td style={{ color: "#f87171", textDecoration: "line-through" }}>{err.lastUserWord}</td>
                        <td><span style={{ background: err.type === "Thiếu từ" ? "rgba(245, 158, 11, 0.2)" : err.type === "Sai chính tả" ? "rgba(56, 189, 248, 0.2)" : "rgba(239, 68, 68, 0.2)", color: err.type === "Thiếu từ" ? "#fbbf24" : err.type === "Sai chính tả" ? "#7dd3fc" : "#fca5a5", padding: "4px 8px", borderRadius: "4px", fontSize: "12px", fontWeight: "bold" }}>{err.type}</span></td>
                        <td><strong style={{ color: "#fff", fontSize: "16px" }}>{err.count}</strong> lần</td>
                        <td><button className="btn-hover" onClick={() => reviewErrorSentence(err.lessonId, err.sentenceIdx)} style={{ background: "linear-gradient(135deg, #ef4444, #b91c1c)", color: "#fff", border: "none", padding: "6px 12px", borderRadius: "6px", cursor: "pointer", fontWeight: "bold", fontSize: "12px" }}>🎯 Ôn lại</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {errorLog.length > 0 && <div style={{ marginTop: "20px", textAlign: "right" }}><button className="btn-hover" onClick={() => {if(window.confirm("Xóa lịch sử lỗi?")) setErrorLog([])}} style={{ background: "transparent", color: "#64748b", border: "1px solid #64748b", padding: "8px 15px", borderRadius: "8px", cursor: "pointer" }}>Xóa toàn bộ</button></div>}
          </div>
        )}
        ...
        {activeTab === "SAVED" && (
          <div className="fade-in glass-panel" style={{ padding: "30px", borderRadius: "16px" }}>
            <h3 style={{ color: "#f59e0b", marginTop: 0, fontSize: '24px', display: 'flex', alignItems: 'center', gap: '8px' }}><span>⭐</span> Danh Sách Ôn Tập ({savedSentences.length} câu)</h3>
            {savedSentences.length === 0 ? <p style={{ color: "#64748b", fontStyle: "italic", textAlign: 'center', padding: '20px' }}>Bấm biểu tượng ⭐ khi luyện nghe để lưu câu khó vào đây.</p> : (
              <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                {savedSentences.map((item) => (
                  <div key={item.id} className="btn-hover" style={{ background: "rgba(0,0,0,0.3)", padding: "20px", borderRadius: "12px", borderLeft: "4px solid #f59e0b", position: 'relative' }}>
                    <button onClick={() => setSavedSentences((prev) => prev.filter((s) => s.id !== item.id))} style={{ position: "absolute", top: "15px", right: "15px", background: "rgba(239, 68, 68, 0.1)", border: "none", color: "#ef4444", width: '30px', height: '30px', borderRadius: '50%', cursor: "pointer", display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✖</button>
                    <div style={{ marginBottom: '10px' }}><span style={{ background: 'rgba(245, 158, 11, 0.2)', color: '#fcd34d', padding: '4px 8px', borderRadius: '4px', fontSize: '12px', fontWeight: 'bold' }}>{item.lessonName}</span><span style={{ color: '#94a3b8', fontSize: '13px', marginLeft: '10px' }}>Câu {item.sentenceIdx + 1}</span></div>
                    <p style={{ margin: "0 0 8px 0", fontSize: "18px", color: "#e2e8f0", paddingRight: '30px', lineHeight: '1.5' }}>{item.transcript}</p>
                    <p style={{ margin: "0 0 15px 0", color: "#94a3b8", fontStyle: 'italic', fontSize: "14px" }}>🇻🇳 {item.translation}</p>
                    <button onClick={() => playSavedSentence(item.lessonId, item.sentenceIdx)} style={{ padding: "8px 16px", background: "linear-gradient(135deg, #3b82f6, #2563eb)", color: "#fff", border: "none", borderRadius: "6px", fontWeight: "bold", cursor: "pointer" }}>🎧 Nghe lại câu này</button>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeTab === "VOCAB" && (
          <div className="fade-in glass-panel" style={{ padding: "30px", borderRadius: "16px" }}>
            <h3 style={{ color: "#a855f7", marginTop: 0, fontSize: '22px', display: 'flex', alignItems: 'center', gap: '8px' }}><span>➕</span> Thêm từ mới</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: "15px", marginBottom: "40px", background: 'rgba(0,0,0,0.2)', padding: '20px', borderRadius: '12px' }}>
              ...
              <button className="btn-hover" onClick={saveVocab} style={{ padding: "14px", background: "linear-gradient(135deg, #a855f7, #7e22ce)", color: "#fff", border: "none", borderRadius: "10px", fontWeight: "bold", fontSize: "18px", cursor: "pointer", marginTop: '5px' }}>💾 Lưu Vào Sổ Tay</button>
            </div>
            <h3 style={{ color: "#38bdf8", fontSize: '22px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '10px' }}>📚 Sổ từ vựng ({vocabList.length} từ)</h3>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "20px", marginTop: '20px' }}>
              {vocabList.map((v) => (
                <div className="btn-hover" key={v.id} style={{ background: "rgba(0,0,0,0.3)", padding: "20px", borderRadius: "12px", borderTop: "4px solid #a855f7", position: "relative" }}>
                  <button onClick={() => deleteVocab(v.id)} style={{ position: "absolute", top: "10px", right: "10px", background: "rgba(239, 68, 68, 0.1)", border: "none", color: "#ef4444", width: '30px', height: '30px', borderRadius: '50%', fontSize: "14px", cursor: "pointer" }}>✖</button>
                  <p style={{ margin: "0 0 8px 0", fontSize: "20px", fontWeight: "bold", color: "#f8fafc" }}>{v.word}</p>
                  ...
                  {v.example && <div style={{ borderLeft: '2px solid #64748b', paddingLeft: '10px' }}><p style={{ margin: 0, fontStyle: "italic", color: "#cbd5e1", fontSize: "14px" }}>"{v.example}"</p></div>}
                </div>
              ))}
            </div>
          </div>
